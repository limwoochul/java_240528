...
# 워크스페이스에서 DB명 우클릭 Drop Schema... 클릭 후 Drop now 클릭
...
# create database if not exists student;   -> 큰 의미 없어서 주석처리

# 데이터베이스 수정 - 이름은 수정이 안됨. 문자 집합을 수정
# ALTER SCHEMA DB명 기본문자집합 바꿀문자집합 기본COLLATE 바꿀COLLATE;
# 워크스페이스에서 DB명 호버하고 2번째 아이콘 클릭 후 원하는 문자집합과 collate를 선택 후 apply
