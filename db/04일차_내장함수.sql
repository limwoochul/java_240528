...
# CONCAT(문자열1, 문자열2, ...) : 문자열들을 이어서 하나의 문자열을 만듦
SELECT CONCAT('안녕', '하', '세요', '.');

# 탐색
# FIELD(찾을문자열, 문자열1, 문자열2, ...) : 문자열들에서 찾을 문자열이 몇 번째에 있는지 반환
# 1부터 시작. 없으면 0
...
# RPAD(문자열, 자리, 채울문자열) : 문자열이 자리수보다 작으면 왼쪽에 채울문자열을 추가하여 자리수를 맞춤
...
SELECT SUBSTRING('TEXT.JPG', 1, 4), SUBSTRING('TEST.JPG', 6, 3);
...
# CEIL(숫자) : 소수점 첫 번째 자리 올림.
# ROUND(숫자) : 소수점 첫 번째 자리 반올림.
# ABS(숫자) : 절댓값
