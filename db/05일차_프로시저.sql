...
CALL UPDATE_POINT('2024160001', 2, 90, 80, 70, 50);
...
BEGIN
    ...
    # 조조영화 여부를 가져옴(1:조조, 0: 아님)
    
    SET _MORNING = (SELECT SD_EARLY FROM SCHEDULE WHERE SD_NUM = _SD_NUM);
    
    CASE _MORNING
		WHEN 1 THEN
		SET _ADULT_PRICE = (SELECT PR_PRICE * _ADULT FROM PRICE WHERE PR_TYPE = '조조 성인');
        SET _TEENAGER_PRICE = (SELECT PR_PRICE * TEENAGER FROM PRICE WHERE PR_TYPE = '조조 청소년');
	ELSE
