...
SELECT CA_NAME 카테고리, FORMAT(IFNULL(AVG(PR_PRICE), 0), 0) 평균가격 FROM CATEGORY 
LEFT JOIN PRODUCT ON PR_CA_NUM = CA_NUM 
GROUP BY CA_NUM;

# 카테고리별 제품의 평균 가격이 3만원 이상인 카테고리를 조회
# (집계함수는 HAVING절에 사용해야함)
SELECT CA_NAME 카테고리, FORMAT(IFNULL(AVG(PR_PRICE), 0), 0) 평균가격 FROM CATEGORY 
LEFT JOIN PRODUCT ON PR_CA_NUM = CA_NUM 
GROUP BY CA_NUM
...
# 카테고리별 등록된 제품수를 조회
SELECT CA_NAME 카테고리, IFNULL(COUNT(PR_CA_NUM), 0) 제품수 FROM CATEGORY 
LEFT JOIN PRODUCT ON PR_CA_NUM = CA_NUM 
GROUP BY CA_NUM;

# 'abc123'회원이 구매한 제품 정보를 조회
SELECT PRODUCT.* FROM BUY 
JOIN PRODUCT ON BU_PR_CODE = PR_CODE 
WHERE BU_ME_ID = 'abc123' AND BU_STATE IN('구매', '구매 확정');

# 'abc123'회원의 누적 구매 금액을 조회
SELECT BU_ME_ID 아이디, FORMAT(SUM(PR_PRICE * BU_AMOUNT), 0) 누적구매금액 FROM BUY 
JOIN PRODUCT ON BU_PR_CODE = PR_CODE 
WHERE BU_ME_ID = 'abc123' AND BU_STATE IN('구매', '구매 확정');
...
# 'abc123'회원이 장바구니에 있는 CLO001제품을 구매했을 때 쿼리
