...
public class StreamEx01 {
	...
	public static void main(String[] args) {
		...
		stream = list.stream();
		//map이 T를 가공해서 R로 만듬. mapToInt는 T를 가공해서 Integer로 만듦
		/* mapToInt는 OptionalDouble로 반환하는데 이유는 객체가 없어서 평균을 구할 수 없으면
		 * 객체를 통해 알 수 있는 메소드를 제공하기 때문에 사용 */
		OptionalDouble ageAvg = stream.mapToInt(Person::getAge).average();
		
		if(ageAvg.isPresent()) {
			System.out.println("나이의 평균 : " + ageAvg.getAsDouble());
		}
		else {
