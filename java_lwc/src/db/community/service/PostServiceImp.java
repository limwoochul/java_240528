...
import db.community.pagenation.Criteria;
...
public class PostServiceImp implements PostService{
	...
	public boolean insertCommunity(String community) {
		...
		//다오에게 커뮤니티명을 주고 db에 추가하라고 시킨 후 성공 여부를 리턴
	...
	public boolean updateCommunity(String oldName, String newName) {
		...
		if(oldName==null || newName==null || newName.trim().length()==0) return false;
		...
		if(newVo != null) return false;
		
		//이전 커뮤니티VO의 커뮤니티명을 이후 커뮤니티명으로 수정
		oldVo.setCo_name(newName.trim());
		
		//다오에게 이전 커뮤니티VO를 주면서 커뮤니티명을 수정하라고 요청하고 처리 여부를 반환
	...
	public boolean insertPost(PostVO post) {
		//게시글VO null 체크해서 문제 있으면 false를 리턴
		if(post == null) return false;
		
		//제목 null체크, 빈문자열 체크해서 문제 있으면 false를 리턴
		if(!checkString(post.getPo_title())) return false;
		
		//내용 null체크, 빈문자열 체크해서 문제 있으면 false를 리턴
		if(!checkString(post.getPo_content())) return false;
	...
	public boolean insertComment(CommentVO comment) {
		...
		if(!checkString(comment.getCm_content())) return false;
