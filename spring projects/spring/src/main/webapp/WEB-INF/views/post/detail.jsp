...
<body>
	...
	<c:if test="${post eq null}">
		<h3 class="mt-3 mb-3">삭제되거나 잘못된 게시글입니다.</h3>
	</c:if>
	...
	2. 수정(댓글 옆) 버튼 클릭이벤트를 등록
	  ...
	  - 댓글 번호와 댓글 내용을 이용해서 문자열로 된 html 코드를 구성
	    => 현재 댓글이 들어간 textarea태그를 포함한 html 코드
	  - 기존에 있는 댓글 등록 화면을 감춤
	  - 기존에 있는 댓글 등록 화면 뒤에 위에서 만든 html코드를 추가
	...
	*/
	/*
	pagination.jsp
	1. 댓글번호를 알아야 삭제할 수 있음
	- 삭제 버튼을 눌렀을 때 댓글 번호를 알아야 함
	 => 삭제 버튼에 댓글번호를 추가
	 => data-xxx
	2. 삭제버튼을 클릭 했을 때 댓글 번호를 서버에(컨트롤러) 주면서 삭제하고 삭제 여부를 알려달라고 요청
	 => 삭제할 댓글 번호를 가져옴. data-xxx
	 => ajax로 댓글 번호를 전송
